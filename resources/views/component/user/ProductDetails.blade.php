...
<script>
    ...
    async function productDetails() {
        ...
        let baseUrl = "{{ url('/') }}"; // Use Laravel's `url('/')` to get the base URL
        document.getElementById('product_img1').src = `${baseUrl}/${Details[0]['img1']}`;
       

        document.getElementById('p_title').innerText=Details[0]['product']['name'];
        document.getElementById('p_price').innerText=`BDT ${Details[0]['product']['price']}`;
    ...
    async function AddToCart() {
        try {
            ...
            let res = await axios.post("/CreateCartList",{
                ...
                if(res.status===200){
                     alert("Proudct Added To Cart Successfully");
